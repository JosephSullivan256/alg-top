...
use crate::tools::kuhn_to_pos;

pub fn run() {
    let window = Window::new(WindowSettings {
        title: "Implicit Manifolds".to_string(),
    ...
    let (pos, ind) = kuhn_to_pos(Mat::new(3,1,vec![-0.5, -0.5, -0.5]));
    let cpu_mesh = CpuMesh {
        positions: Positions::F32(pos),
        indices: Indices::U16(ind),
        ..Default::default()
    };
    // cpu_mesh
    ...
    window.render_loop(move |mut frame_input| {
        ...
        if redraw {
            frame_input
                ...
                .render(
                    &camera,
                    vertices.into_iter().chain(&edges),
